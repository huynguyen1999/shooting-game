...
export class BulletSpeedUpCommand extends Command {
  public _id: string;
  ...
  execute(): void {
    const newSpeed = this.receiver.bullet_speed * this.speed_up_ratio;
    this.speed_diff = newSpeed - this.receiver.bullet_speed;
    this.receiver.bullet_speed = newSpeed;
    setTimeout(() => this.undo(), this.effect_duration);
  }
  public undo(): void {
    this.receiver.bullet_speed -= this.speed_diff;
  }
}
